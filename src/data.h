...
// T0: clockRefreshRtc post-read sanity check entry point — caller passes
// validation result; if RTC chip read returns out-of-range fields (uninit
// BM8563 / setDateTime silently failed / cosmic ray), force-disable clocking
// path so drawClock doesn't print garbage like "42949 / jan 429".
inline void dataRtcInvalidate() { _rtcValid = false; }
// T0 real-fix: setup() may write a placeholder RTC value (2026-01-01 00:00:00)
// if BM8563 returns garbage at boot, then call this to enable clocking path
// even before Hardware Buddy sends real time JSON. User sees a valid clock
// immediately (placeholder) → real time replaces on first heartbeat.
inline void dataRtcForceValid() { _rtcValid = true; }
...
static void _applyJson(const char* line, TamaState* out) {
  ...
  if (!t.isNull() && t.size() == 2) {
    ...
    // M5Unified RTC API (replaces M5StickCPlus RTC_TimeTypeDef/Set):
    m5::rtc_datetime_t rdt;
    rdt.time.hours = lt.tm_hour; rdt.time.minutes = lt.tm_min; rdt.time.seconds = lt.tm_sec;
    rdt.date.year  = lt.tm_year + 1900; rdt.date.month = lt.tm_mon + 1;
    rdt.date.date  = lt.tm_mday;        rdt.date.weekDay = lt.tm_wday;
...
// G25 follow-up: _usbLine removed (was dead BSS after USB-CDC feed deletion).
// Only BLE (_btLine) is the documented transport per references.md §A.1.
...
inline void dataPoll(TamaState* out) {
  ...
  // G25: _usbLine.feed(Serial, out) PERMANENTLY DELETED on this clawstick port.
  // Upstream's data.h was designed for hardware UART Serial on M5StickCPlus.
  // Our M5StickS3 build sets -DARDUINO_USB_CDC_ON_BOOT=1, mapping Serial to
  // USB-CDC. The `while (Serial.available()) { Serial.read(); ... }` loop in
  // _LineBuf::feed() spins indefinitely on USB-CDC — confirmed via bisect
  // 2026-05-04: loop reaches "after M5.update()" then hangs in dataPoll. We
  // do NOT use USB-CDC for the wire protocol (BLE NUS only); USB-CDC is for
  // diagnostic Serial.printf only. Removing this is safe + correct per
  // references.md §A.1 (only BLE NUS is the documented wire transport).
  // Do NOT re-enable without first guarding behind a USB-host-active check.
