...
bool characterInit(const char* name) {
  // G22: upstream uses begin(false) which assumes fs.img was uploaded via
  // `pio run -t uploadfs`. Our deploy flow doesn't pre-upload an fs.img, so
  // first boot finds corrupt/empty spiffs partition → mount fails → setup
  // hangs → splash sticks (per serial output: "Corrupted dir pair at {0x0,0x1}
  // mount failed (-84)"). Auto-format on first-boot failure makes deploy
  // self-contained. Subsequent character pack uploads via xfer.h still work
  // because mount succeeds after format.
  if (!LittleFS.begin(true)) {   // formatOnFail = true
    Serial.println("[char] LittleFS mount failed even after format attempt");
    return false;
  }
  ...
  // G24: verify each manifest-listed GIF actually exists on LittleFS before
  // declaring loaded=true. Half-uploaded packs (xfer.h "char_begin/file/chunk"
  // protocol per references.md §A.1) leave manifest.json present but GIF
  // files missing or truncated → characterSetState's gif.open fails → gifOpen
  // stays false → characterTick early-returns without drawing → sprite stays
  // at last fillSprite color → pushSprite shows uniform-color screen. Falling
  // back to loaded=false → buddyMode=true (main.cpp:985) → buddyTick draws
  // hardcoded ASCII species (per references.md §A.1 "18 ASCII pet species" =
  // upstream's primary identity, GIF is optional augmentation).
  // Note: G21 splash-clear was reverted per G27; reference left here as
  // documentation of original failure mode that motivated this defense.
  if (gifTotal == 0) {
    Serial.println("[char] manifest has no states with frames — falling back to ASCII");
    return false;
  }
  char full[80];
  for (uint8_t i = 0; i < gifTotal; i++) {
    snprintf(full, sizeof(full), "%s/%s", basePath, gifPaths[i]);
    File probe = LittleFS.open(full, "r");
    if (!probe) {
      Serial.printf("[char] missing GIF: %s — falling back to ASCII\n", full);
      return false;
    }
    // Truncated upload (xfer "file" sent but no "chunk") → 0-byte file passes
    // open() probe but later gif.open() fails. Treat as missing.
    size_t sz = probe.size();
    probe.close();
    if (sz == 0) {
      Serial.printf("[char] zero-byte GIF: %s — falling back to ASCII\n", full);
      return false;
    }
  }

  loaded = true;
  Serial.printf("[char] loaded '%s' from %s (%d GIFs verified)\n",
...
void characterSetState(uint8_t s) {
  ...
  if (gif.open(full, gifOpenCb, gifCloseCb, gifReadCb, gifSeekCb, gifDrawCb)) {
    ...
    // G24 root cause: gif.open() runtime failure (file present but format
    // incompat / AnimatedGIF lib edge / heap fragment) leaves gifOpen=false.
    // characterTick (line 397-403) early-returns → sprite holds last
    // fillSprite(pal.bg=BLACK by default) → pushSprite推黑屏 forever.
    //
    // Self-heal: close character + flip buddyMode=true → next loop iteration
    // (main.cpp:1226-1227) routes to buddyTick which draws hardcoded ASCII
    // species (per references.md §A.1 "18 ASCII pet species" = upstream
    // primary identity, GIF is optional). User sees something rather than
    // black; D6 acceptance ("板屏显 pending") preserved even when GIF fails.
    Serial.printf("[char] open failed: %s (err %d) — self-heal: ASCII fallback\n",
    ...
    curState = 0xFF;
    buddyMode = true;
    gifAvailable = false;     // mirror loaded — nextPet (main.cpp:84) won't cycle into broken GIF
    // M2: would also speciesIdxSave(0) for sticky reboot recovery, but stats.h
    // is single-TU only (#include guard says "main.cpp only — duplicate symbols
    // otherwise"). On reboot characterInit re-runs Fix A; if files still pass
    // probe + non-zero but gif.open still fails, self-heal fires again. One
    // brief !loaded iteration before buddyTick takes over — harmless on screen.
  }
}
